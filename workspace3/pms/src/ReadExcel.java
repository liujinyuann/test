...
public class ReadExcel {
	public static DataFrom readExcel(DataFrom dataFrom) throws EncryptedDocumentException, IOException {
		...
		double dblShussha;
		double dblTaisha;
		double dblNormalHour = 0;
		double dblNightHour = 0;
		double dblMidNightHour = 0;
...
//	            Pms.setStrYMD(sdf.format(minDate));
	            Pms.setStrYMD(minDate);
			...
			for (int j = 7; j < rows; j++) {
				...
				// 出社を取得する
				dblShussha = row.getCell(getExcelColIndex("E")).getNumericCellValue();
				dataMap.put("Shussha", String.valueOf(dblShussha * 24));
				// 退社を取得する
				dblTaisha = row.getCell(getExcelColIndex("F")).getNumericCellValue();
				dataMap.put("Taisha", String.valueOf(dblTaisha * 24));
				// 休暇を取得する
				dataMap.put("Kyuuka", row.getCell(getExcelColIndex("P")).getStringCellValue());

				dblNormalHour = (dblTaisha - dblShussha)*24 - 1;
				if (dblNormalHour > 8) {
					dblNightHour = dblNormalHour - 8;
					dblNormalHour = 8;
				}
				if (dblNightHour > 4) {
					dblMidNightHour= dblNightHour - 4;
					dblNightHour = 4;
				}
				dataMap.put("NormalHour", String.valueOf(dblNormalHour));
				dataMap.put("NightHour", String.valueOf(dblNightHour));
				dataMap.put("MidNightHour", String.valueOf(dblMidNightHour));
